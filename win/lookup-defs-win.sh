...
# add the PREFIX to the QUERY string.
QUERY="$PREFIX$1" # now QUERY is the search term with the prefix added, so if the user types "docker", QUERY will be "DEF-docker, and therefore ONLY-MATCH the entries to the DEF section like we want.
