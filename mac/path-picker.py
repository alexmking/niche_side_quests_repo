...
# DESCRIPTION OF ALL THE PICKER SCRIPTS SHOULD DO:
# config-picker           set-alias== pickConfig                (opens selection for editing in vs code)      ✅DONE✅
# path-picker             set-alias== pickPath                  (copies selection to clipboard)               ✅DONE✅
# dir-picker              set-alias== pickDir                   (open new finder window at selection )          ✅DONE✅
# file-picker             set-alias== pickFile                  (opens sel file but not necc for editing (like EXCELdoc foe example)) (SOME OVERLAP W pickConfig(), may be a problem idk..)
# everything-picker       set-alias== pickEverything            (NEW one that combines all dirs/files/configs etc and does most-likely action based on the selection type (so if you sel a DIR then itll open finderwindow at that dir, but if you sel a excel file itll just open normally etc etc ))) (SOME OVERLAP W pickConfig(), may be a problem idk..)
...
import subprocess
path_to_paste = sys.argv[1]
subprocess.run(["pbcopy"], input=path_to_paste.encode())



####### COPY THIS SCRIPT TO...
# we can use this exact script for many of the other 'smart' fzf/dropdown based search systems we want to build out, such as:
# config files (where sel == opens file in code editor, instead of copying path)
# open fileExpl window at selected location, but the selection is fuzzyFind/dropdown across a hard-coded collection of my most common/useful directory spots
...
# ❗TODO❗
# ❗TODO❗ 
# DECIDE ON THE NAMING CONVENTION FOR ALL THESE PATH-SEARCH-BASED SCRIPTS, SUCH AS:
# all-paths, adv-paths, smart-paths, fzf-paths, find-path, … (❓) path-finder, dir-finder, finder-path??
# ❗TODO❗
# ❗TODO❗
# ❗TODO❗
# ❗TODO❗
